...
def button_reward_fn(act: torch.Tensor, next_model_state: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    # Delta reward: Toward button pressed
    if "prev_obs" in next_model_state:
        prev_obs = next_model_state["prev_obs"]
        target_pos = next_model_state["init_data"]["button"].target_pos
        prev_dist = torch.abs(target_pos[:, 2] - prev_obs[:, 6]) # button Z is index 6
        curr_dist = torch.abs(target_pos[:, 2] - next_obs[:, 6])
        reward += 20.0 * (prev_dist - curr_dist)

    # Control penalty
    reward -= 0.1 * torch.norm(act, dim=-1)
...
def door_reward_fn(act: torch.Tensor, next_model_state: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    # Delta reward: Toward opening door
    if "prev_obs" in next_model_state:
        prev_obs = next_model_state["prev_obs"]
        prev_dist = torch.abs(target_pos_x - prev_obs[:, 7])
        curr_dist = torch.abs(target_pos_x - next_obs[:, 7])
        reward += 20.0 * (prev_dist - curr_dist)

    # Baseline subtraction (~0.54) and control penalty
    reward = reward - 0.54 - 0.1 * torch.norm(act, dim=-1)
    return reward.view(-1, 1)
...
def window_reward_fn(act: torch.Tensor, next_model_state: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    # Delta reward: Toward sliding window
    if "prev_obs" in next_model_state:
        prev_obs = next_model_state["prev_obs"]
        target_y = next_model_state["init_data"]["window"].target_pos[:, 1]
        prev_dist = torch.abs(prev_obs[:, 12] - target_y) # window handle Y is index 12
        curr_dist = torch.abs(next_obs[:, 12] - target_y)
        reward += 20.0 * (prev_dist - curr_dist)

    # Baseline subtraction (~2.62) and control penalty
    reward = reward - 2.62 - 0.1 * torch.norm(act, dim=-1)
    return reward.view(-1, 1)
...
def faucet_reward_fn(act: torch.Tensor, next_model_state: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    # Delta reward: Toward target
    if "prev_obs" in next_model_state:
        prev_obs = next_model_state["prev_obs"]
        prev_obj = prev_obs[:, 14:17]
        prev_dist = torch.norm(prev_obj - next_model_state["init_data"]["faucet"].target_pos, dim=-1)
        # target_to_obj is already curr_dist
        reward += 20.0 * (prev_dist - target_to_obj)

    # Baseline subtraction (~0.82) and control penalty
    reward = reward - 0.82 - 0.1 * torch.norm(act, dim=-1)
    return reward.view(-1, 1)
...
def peg_reward_fn(act: torch.Tensor, next_model_state: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    if "prev_obs" in next_model_state:
        prev_obs = next_model_state["prev_obs"]
        prev_dist = torch.norm(prev_obs[:, 17:20] - target, dim=-1)
        reward += 20.0 * (prev_dist - obj_to_target)

    # Baseline subtraction (~0.10) and control penalty
    reward = reward - 0.10 - 0.1 * torch.norm(act, dim=-1)
    return reward.view(-1, 1)
...
def block_reward_fn(act: torch.Tensor, next_model_state: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    # Delta reward: toward midpoint (first goal)
    if "prev_obs" in next_model_state:
        prev_obs = next_model_state["prev_obs"]
        prev_dist = torch.norm((prev_obs[:, 20:23] - midpoint) * in_place_scaling, dim=-1)
        reward += 20.0 * (prev_dist - obj_to_midpoint)

    # Baseline subtraction (~0.07) and control penalty
    reward = reward - 0.07 - 0.1 * torch.norm(act, dim=-1)
    return reward.view(-1, 1)
